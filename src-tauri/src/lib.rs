use ashpd::desktop::{
  PersistMode, Session,
  remote_desktop::{
    Axis, DeviceType, KeyState, NotifyPointerAxisDiscreteOptions, NotifyPointerButtonOptions,
    NotifyKeyboardKeysymOptions, RemoteDesktop, SelectDevicesOptions,
  },
};
use keyring::Entry;
...
fn voice_runtime_data_dir() -> Result<PathBuf, String> {
  let home = env::var_os("HOME").ok_or_else(|| "Could not find the home directory for the voice runtime.".to_string())?;
...
fn voice_model_dir() -> Result<PathBuf, String> {
  Ok(voice_runtime_data_dir()?.join("models").join("vosk-model-small-en-us-0.15"))
}

fn voice_runtime_installed() -> Result<bool, String> {
  Ok(
    voice_model_dir()?.is_dir()
      && voice_runtime_data_dir()?.join(".openwakeword-silero-ready").is_file(),
  )
}

fn voice_runtime_upgrade_ready() -> Result<bool, String> {
  Ok(voice_runtime_data_dir()?.join(".openwakeword-silero-ready").is_file())
}
...
fn whisper_model() -> Result<PathBuf, String> {
  Ok(voice_runtime_data_dir()?.join("models").join("ggml-base.en.bin"))
}

fn high_accuracy_voice_available() -> bool {
  whisper_binary().is_ok_and(|binary| binary.is_file()) && whisper_model().is_ok_and(|model| model.is_file())
}
...
fn save_voice_listener_config_file(config: VoiceListenerConfig) -> Result<(), String> {
  let config = VoiceListenerConfig {
    ...
    open_phrases: normalize_voice_command_phrases(config.open_phrases, &["open", "launch", "start"]),
    close_phrases: normalize_voice_command_phrases(config.close_phrases, &["close", "quit", "exit"]),
  };
...
fn ensure_voice_listener_config() -> Result<PathBuf, String> {
  ...
  let config = fs::read_to_string(&config_file)
    ...
    .unwrap_or(VoiceListenerConfig {
      ...
      open_phrases: vec!["open".to_string(), "launch".to_string(), "start".to_string()],
...
fn listener_is_running(listener: &VoiceListener) -> Result<bool, String> {
  ...
  match process.try_wait().map_err(|error| format!("Could not inspect the voice listener: {error}"))? {
    Some(_) => {
      *child = None;
      Ok(false)
    }
...
fn emit_voice_runtime_status(app: &AppHandle, installed: bool, listening: bool, message: impl Into<String>) {
  let _ = app.emit(
...
fn desktop_application_id(application: &str) -> Result<String, String> {
  ...
  for directory in [PathBuf::from("/usr/share/applications"), env::var_os("HOME").map(|home| PathBuf::from(home).join(".local/share/applications")).unwrap_or_default()] {
  ...
  partial_match.ok_or_else(|| format!("I could not find an installed application named {application}."))
}
...
fn inspect_codex_runtime() -> CodexRuntimeStatus {
  let Some(binary) = codex_binary() else {
    return CodexRuntimeStatus {
      ...
      version: None,
      binary_path: None,
      message: "Codex CLI is not installed. Install it, then refresh this status.".to_string(),
    };
  };
...
fn inspect_ollama_runtime() -> OllamaRuntimeStatus {
  ...
  match ollama_request("GET", "/api/tags", None) {
    ...
    Err(error) => OllamaRuntimeStatus {
      ...
      endpoint: OLLAMA_ENDPOINT.to_string(),
      models: Vec::new(),
      message: format!("Ollama is reachable, but its local model catalog could not be read: {error}"),
    },
  }
}
...
fn resolve_workspace(input: &str) -> Result<PathBuf, String> {
  ...
  if trimmed.is_empty() {
    return Err(
      "Choose a project workspace in Settings before running an autonomous agent.".to_string(),
    );
  }
...
fn agent_prompt(request: &AgentRunRequest, local_ollama: bool) -> String {
  let memory = if request.memory.trim().is_empty() {
    ...
    format!("Persistent context supplied by the user:\n{}", request.memory.trim())
  };
  ...
  let runtime_capability = if local_ollama {
    ...
    format!("Terminal capability for this agent is `{}`.", request.terminal_access.trim())
  };
...
fn validate_run_safety(request: &AgentRunRequest) -> Result<(), String> {
  ...
  if !matches!(request.file_access.as_str(), "none" | "read" | "write" | "full") {
  ...
  if request.terminal_access == "admin" {
    return Err("Administrator terminal access is blocked by the desktop safety boundary.".to_string());
  }
  ...
  if request.destructive_actions_approved
    && (request.approval_id.is_none()
      || !request.authorized_scopes.iter().any(|scope| scope == "files"))
  {
...
fn relative_workspace_path(input: &str) -> Result<PathBuf, String> {
  ...
  {
    return Err("Workspace tool paths must be relative and stay inside the selected workspace.".to_string());
  }
  Ok(path.to_path_buf())
}
...
fn workspace_tool_path(
  ...
  if relative.components().any(|component| component.as_os_str() == ".git") {
    return Err("The local Ollama agent cannot access the workspace's .git directory.".to_string());
  }
  ...
  match fs::symlink_metadata(&candidate) {
    ...
    Err(error) if error.kind() == std::io::ErrorKind::NotFound && allow_missing => {
      ...
      let resolved_parent = fs::canonicalize(parent)
        .map_err(|_| "The target folder does not exist inside the selected workspace.".to_string())?;
...
fn read_workspace_file(workspace: &Path, path: &str) -> Result<String, String> {
  ...
  let content = if truncated {
    content.chars().take(MAX_OLLAMA_FILE_CHARS).collect::<String>()
  } else {
    content
  };
  Ok(format!(
...
fn write_workspace_file(workspace: &Path, path: &str, content: &str) -> Result<String, String> {
  ...
  if content.chars().count() > MAX_OLLAMA_WRITE_CHARS {
    return Err("The requested file write is too large for the local coding agent.".to_string());
  }
  ...
  fs::write(&file, content).map_err(|error| format!("Could not write the workspace file: {error}"))?;
  Ok(format!("Wrote {}", workspace_display_path(workspace, &file)))
}
...
fn create_workspace_directory(workspace: &Path, path: &str) -> Result<String, String> {
  ...
  Ok(format!("Created {}", workspace_display_path(workspace, &directory)))
}
...
fn execute_ollama_workspace_tool(
  ...
  match tool_call.name.as_str() {
    ...
    "read_workspace_file" => {
      read_workspace_file(workspace, tool_string_argument(&tool_call.arguments, "path")?)
    }
    "write_workspace_file" => {
      if !matches!(request.file_access.as_str(), "write" | "full") {
        return Err("This agent does not have workspace-write access for this run.".to_string());
      }
      write_workspace_file(
        workspace,
    ...
    "create_workspace_directory" => {
      ...
      create_workspace_directory(workspace, tool_string_argument(&tool_call.arguments, "path")?)
    }
    _ => Err(format!("`{}` is not an available local workspace tool.", tool_call.name)),
  }
}
...
fn run_ollama_task(
  ...
  let result = (|| {
    ...
    let installed_model = runtime_status
      ...
      .ok_or_else(|| format!("The Ollama model `{model}` is not installed at {OLLAMA_ENDPOINT}."))?;
    if !installed_model
      .capabilities
      .iter()
    ...
    emit_run_event(
      ...
      &run_id,
      "status",
      format!("Starting local Ollama model {model} in {}", workspace.to_string_lossy()),
    );
...
fn launch_allowed_application(application: String) -> Result<(), String> {
  ...
  if requested == "terminal" {
    let terminal = ["konsole", "kitty", "gnome-terminal", "xterm"]
      ...
      .ok_or_else(|| "No supported terminal application is installed or available in PATH.".to_string())?;
...
fn close_allowed_application(application: String) -> Result<(), String> {
  ...
  if status.success() {
    ...
    Err(format!("{label} is not running or did not accept the close request."))
  }
}

#[tauri::command]
fn desktop_control_status(state: State<'_, DesktopControl>) -> Result<DesktopControlStatus, String> {
  let enabled = state
    .session
    .lock()
...
#[tauri::command]
async fn enable_desktop_control(state: State<'_, DesktopControl>) -> Result<DesktopControlStatus, String> {
  if state
    .session
    .lock()
    .map_err(|_| "The desktop control registry is unavailable.".to_string())?
  ...
  if !selected.devices().contains(DeviceType::Pointer) || !selected.devices().contains(DeviceType::Keyboard) {
  ...
  *state
    .session
    .lock()
    .map_err(|_| "The desktop control registry is unavailable.".to_string())? = Some(Arc::new(DesktopControlSession { portal, session }));
...
async fn send_desktop_pointer_action(
  ...
  let active_session = state
    ...
    .map_err(|_| "The desktop control registry is unavailable.".to_string())?
    .clone()
    .ok_or_else(|| "Enable KDE desktop input before using voice pointer commands.".to_string())?;
  match action.trim().to_ascii_lowercase().as_str() {
    "move-left" => active_session.portal.notify_pointer_motion(&active_session.session, -90.0, 0.0, Default::default()).await,
    "move-right" => active_session.portal.notify_pointer_motion(&active_session.session, 90.0, 0.0, Default::default()).await,
    "move-up" => active_session.portal.notify_pointer_motion(&active_session.session, 0.0, -90.0, Default::default()).await,
    "move-down" => active_session.portal.notify_pointer_motion(&active_session.session, 0.0, 90.0, Default::default()).await,
    "click" => {
      match active_session.portal.notify_pointer_button(&active_session.session, 0x110, KeyState::Pressed, NotifyPointerButtonOptions::default()).await {
        Ok(()) => active_session.portal.notify_pointer_button(&active_session.session, 0x110, KeyState::Released, NotifyPointerButtonOptions::default()).await,
        Err(error) => Err(error),
      }
    }
    "double-click" => {
      let mut result = Ok(());
      for _ in 0..2 {
        result = active_session.portal.notify_pointer_button(&active_session.session, 0x110, KeyState::Pressed, NotifyPointerButtonOptions::default()).await;
        if result.is_ok() {
          result = active_session.portal.notify_pointer_button(&active_session.session, 0x110, KeyState::Released, NotifyPointerButtonOptions::default()).await;
        }
        if result.is_err() {
          break;
        }
    ...
    "scroll-up" => active_session.portal.notify_pointer_axis_discrete(&active_session.session, Axis::Vertical, -3, NotifyPointerAxisDiscreteOptions::default()).await,
    "scroll-down" => active_session.portal.notify_pointer_axis_discrete(&active_session.session, Axis::Vertical, 3, NotifyPointerAxisDiscreteOptions::default()).await,
...
async fn close_active_desktop_application(state: State<'_, DesktopControl>) -> Result<(), String> {
  let active_session = state
    ...
    .map_err(|_| "The desktop control registry is unavailable.".to_string())?
    .clone()
    .ok_or_else(|| "Enable KDE desktop input before closing the active application by voice.".to_string())?;
  let result = async {
    active_session.portal.notify_keyboard_keysym(&active_session.session, KEYSYM_ALT, KeyState::Pressed, NotifyKeyboardKeysymOptions::default()).await?;
    active_session.portal.notify_keyboard_keysym(&active_session.session, 0xffc1, KeyState::Pressed, NotifyKeyboardKeysymOptions::default()).await?;
    active_session.portal.notify_keyboard_keysym(&active_session.session, 0xffc1, KeyState::Released, NotifyKeyboardKeysymOptions::default()).await?;
    active_session.portal.notify_keyboard_keysym(&active_session.session, KEYSYM_ALT, KeyState::Released, NotifyKeyboardKeysymOptions::default()).await
  }
  .await;
...
async fn send_desktop_keyboard_action(
  ...
  let active_session = state
    ...
    .map_err(|_| "The desktop control registry is unavailable.".to_string())?
    .clone()
    .ok_or_else(|| "Enable KDE desktop input before using voice keyboard commands.".to_string())?;
  ...
  let events: Vec<(i32, KeyState)> = match action.as_str() {
    "open-launcher" => vec![(KEYSYM_SUPER, KeyState::Pressed), (KEYSYM_SUPER, KeyState::Released)],
    "volume-up" => vec![(0x1008ff13, KeyState::Pressed), (0x1008ff13, KeyState::Released)],
    "volume-down" => vec![(0x1008ff11, KeyState::Pressed), (0x1008ff11, KeyState::Released)],
    "toggle-mute" => vec![(0x1008ff12, KeyState::Pressed), (0x1008ff12, KeyState::Released)],
    "next-window" => vec![(KEYSYM_ALT, KeyState::Pressed), (0xff09, KeyState::Pressed), (0xff09, KeyState::Released), (KEYSYM_ALT, KeyState::Released)],
    "previous-window" => vec![(KEYSYM_ALT, KeyState::Pressed), (KEYSYM_SHIFT, KeyState::Pressed), (0xff09, KeyState::Pressed), (0xff09, KeyState::Released), (KEYSYM_SHIFT, KeyState::Released), (KEYSYM_ALT, KeyState::Released)],
    ...
    "shift-tab" => vec![(KEYSYM_SHIFT, KeyState::Pressed), (0xff09, KeyState::Pressed), (0xff09, KeyState::Released), (KEYSYM_SHIFT, KeyState::Released)],
    ...
    "select-all" => vec![(KEYSYM_CONTROL, KeyState::Pressed), (0x61, KeyState::Pressed), (0x61, KeyState::Released), (KEYSYM_CONTROL, KeyState::Released)],
    "copy" => vec![(KEYSYM_CONTROL, KeyState::Pressed), (0x63, KeyState::Pressed), (0x63, KeyState::Released), (KEYSYM_CONTROL, KeyState::Released)],
    "cut" => vec![(KEYSYM_CONTROL, KeyState::Pressed), (0x78, KeyState::Pressed), (0x78, KeyState::Released), (KEYSYM_CONTROL, KeyState::Released)],
    "paste" => vec![(KEYSYM_CONTROL, KeyState::Pressed), (0x76, KeyState::Pressed), (0x76, KeyState::Released), (KEYSYM_CONTROL, KeyState::Released)],
    "undo" => vec![(KEYSYM_CONTROL, KeyState::Pressed), (0x7a, KeyState::Pressed), (0x7a, KeyState::Released), (KEYSYM_CONTROL, KeyState::Released)],
    "redo" => vec![(KEYSYM_CONTROL, KeyState::Pressed), (KEYSYM_SHIFT, KeyState::Pressed), (0x7a, KeyState::Pressed), (0x7a, KeyState::Released), (KEYSYM_SHIFT, KeyState::Released), (KEYSYM_CONTROL, KeyState::Released)],
    _ => return Err("That keyboard action is not approved for voice control.".to_string()),
  };
  for (keysym, key_state) in events {
    active_session
      .portal
      .notify_keyboard_keysym(&active_session.session, keysym, key_state, NotifyKeyboardKeysymOptions::default())
      .await
...
fn control_named_desktop_window(
  ...
  if !desktop_control_active {
    return Err("Enable KDE desktop input before controlling a named application window by voice.".to_string());
  }
...
async fn type_desktop_text(
  ...
  if text.len() > 280 || !text.chars().all(|character| character.is_ascii_alphanumeric() || matches!(character, ' ' | '\n' | '-' | '.' | '/' | '_' | ':' | ',' | '=' | '+' | '?' | '@')) {
  ...
  for character in text.chars() {
    let keysym = if character == '\n' { 0xff0d } else { character as i32 };
    active_session
      .portal
      .notify_keyboard_keysym(&active_session.session, keysym, KeyState::Pressed, NotifyKeyboardKeysymOptions::default())
      .await
      .map_err(|error| format!("KDE could not type the dictated text: {error}"))?;
    active_session
      .portal
      .notify_keyboard_keysym(&active_session.session, keysym, KeyState::Released, NotifyKeyboardKeysymOptions::default())
      .await
      .map_err(|error| format!("KDE could not type the dictated text: {error}"))?;
  }
  Ok(())
...
fn install_voice_runtime(app: AppHandle) -> Result<(), String> {
  ...
  emit_voice_runtime_status(&app, false, false, "Downloading and installing the offline voice engine...");
  thread::spawn(move || {
...
fn install_high_accuracy_voice_runtime(app: AppHandle) -> Result<(), String> {
  if high_accuracy_voice_available() {
    emit_voice_runtime_status(&app, true, false, "High-accuracy offline voice is already installed.");
    return Ok(());
  }
  let setup_script = voice_runtime_file(&app, "setup-high-accuracy.sh")?;
  let runtime_dir = voice_runtime_data_dir()?;
  emit_voice_runtime_status(&app, voice_model_dir()?.is_dir(), false, "Building the high-accuracy offline speech engine and downloading its model...");
  thread::spawn(move || {
    let result = Command::new("bash")
      .arg(setup_script)
      .env("VOICE_RUNTIME_DIR", &runtime_dir)
    ...
    match result {
      ...
      Ok(output) => emit_voice_runtime_status(
        ...
        false,
        format!("High-accuracy voice installation failed: {}", String::from_utf8_lossy(&output.stderr).trim()),
      ),
      Err(error) => emit_voice_runtime_status(
        &app,
        false,
...
fn start_voice_listener(app: AppHandle, state: State<'_, VoiceListener>) -> Result<(), String> {
  ...
  if !model.is_dir() {
    return Err("Offline voice is not installed. Select Install offline voice engine first.".to_string());
  }
  let runtime_dir = voice_runtime_data_dir()?;
  let python = runtime_dir.join("venv").join("bin").join("python");
  if !python.is_file() {
    return Err("The offline voice installation is incomplete. Install the voice engine again.".to_string());
  }
  ...
  let stdout = child.stdout.take().ok_or_else(|| "The offline voice listener did not provide output.".to_string())?;
  {
    ...
    for line in BufReader::new(stdout).lines().map_while(Result::ok) {
      let event = match serde_json::from_str::<VoiceTranscriptEvent>(&line) {
        ...
        Err(_) => VoiceTranscriptEvent {
          ...
          transcript: format!("Lucy returned an invalid runtime message: {}", line.trim()),
        },
      };
...
pub fn run() {
  tauri::Builder::default()
    .plugin(tauri_plugin_single_instance::init(|app, _, _| open_voice_control(app)))
    ...
    .setup(|app| {
      ...
      let quit = MenuItem::with_id(app, "quit", "Quit AI Agent Control Center", true, None::<&str>)?;
